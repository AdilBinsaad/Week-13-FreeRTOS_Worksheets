...
- Binary semaphore → มีค่า สูงสุด 1 ให้เกินก็ไม่เพิ่ม
- Counting semaphore → จะเพิ่มค่าได้ตาม max count
2. ISR สามารถใช้ `xSemaphoreGive` หรือต้องใช้ `xSemaphoreGiveFromISR`?
- ต้องใช้ xSemaphoreGiveFromISR() เพราะ API ปกติไม่ปลอดภัยใน interrupt context
3. Binary Semaphore แตกต่างจาก Queue อย่างไร?
- Binary semaphore → แค่สัญญาณ 0/1 ใช้ sync ระหว่าง tasks
- Queue → เก็บ ข้อมูลหลายตัว (FIFO) ใช้ทั้ง sync และส่งข้อมูล
