...
- ใช้ Queue Set หรือ xQueueSelectFromSet() จะบอกว่า task ควรอ่านข้อมูลจาก queue ไหน
2. เมื่อหลาย Queue มีข้อมูลพร้อมกัน เลือกประมวลผลอันไหนก่อน?
- Scheduler จะเลือก queue ที่มีข้อมูลมาก่อนหรือ priority สูงสุด (ขึ้นกับการ implement ของ queue set)
3. Queue Sets ช่วยประหยัด CPU อย่างไร?
- Task รอครั้งเดียว แทนต้องตรวจหลาย queue ทีละอัน → ลด busy-wait และให้ CPU ไปทำ task อื่นได้
